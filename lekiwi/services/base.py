...
class ServiceEvent:
    def __init__(
        self, event_type: str, payload: Any, priority: Priority = Priority.NORMAL
    ):
...
class ServiceBase(ABC):
    ...
    def dispatch(
        ...
        if not self._running.is_set():
            self.logger.warning(
                f"Service {self.name} is not running, ignoring event {event_type}"
            )
            return
        ...
        with self._event_lock:
            if (
                ...
                or event.priority <= self._current_event.priority
            ):
                self._current_event = event
                self._event_available.set()

        self.logger.debug(
            f"Dispatched event {event_type} with priority {priority.name}"
        )
