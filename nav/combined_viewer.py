...
def draw_pose_overlay(frame, result, event, is_fall: bool, fps: float, quality: dict, face_stats: dict):
    ...
    cv2.putText(frame, f"FPS {fps:.1f}", (10, 215), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
...
def main():
    ...
    while cap.isOpened():
        ...
        draw_pose_overlay(vis_frame, result, event or last_event, is_fall, fps, quality, face_stats)
        ...
        if scores[1] > THRESHOLD:
            cv2.putText(vis_frame, "OBSTACLE DETECTED!", (center_x - 150, center_y - 50), 
        ...
        cv2.arrowedLine(vis_frame, (center_x, center_y + 50), (end_x, end_y), arrow_color, 4)
        
        # Add MonoPilot Nav Info
        cv2.putText(vis_frame, f"NAV: vx={vx:.2f} vy={vy:.2f} rot={omega:.2f}", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(vis_frame, f"Depth: L={scores[0]:.0f} C={scores[1]:.0f} R={scores[2]:.0f}", (10, 90), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        ...
        cv2.putText(dmap_color, f"{scores[0]:.0f}", (third//2 - 20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
        cv2.putText(dmap_color, f"{scores[1]:.0f}", (third + third//2 - 20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
        cv2.putText(dmap_color, f"{scores[2]:.0f}", (2*third + third//2 - 20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
